...
bool wave()
{
    ...
    cout << "[ Log ]"
         << "[ " << mytime << " ] "
         << "[ Recv ]"
         << "Ack:" << 0 << " Len:" << ack_p.getLen() << " Checksum:" << 49550 << endl;
    cout << "[ Log ]"
         << "[ " << mytime << " ] "
         << "[ Info ]"
         << "二次挥手成功!" << endl;
...
void readfile()
{
    ...
    while (1)
    {
        ...
        send_packet.setSeq(0);
        send_packet.setLen(packet_num);
...
DWORD WINAPI recvAck(LPVOID Socketindex)
{
    while (is_send)
    {
        UDP_PACKET recv_packet;
        my_recvfrom(recv_packet);
        if (recv_packet.getType() == 1 && recv_packet.getAck() != 0)
        {
            getime(mytime);
            while (is_log1)
                ;
            ...
            {
                ackpa[recv_packet.getAck() - 1] = 1;
                while (ackpa[win_left] != 0)
                {
                    win_left++;
                    if (win_left + window >= packet_num)
                        win_right = packet_num;
                    else
                        win_right = win_left + window;
                    getime(mytime);
                    while (is_log1)
                        ;
                    is_log2 = 1;
